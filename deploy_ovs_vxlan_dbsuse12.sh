#!/bin/bash
# -------------------------------------------
# Name    : deploy_ovs_vxlan_dbsuse12.sh
...
ip link show br0 && ovs-vsctl del-br br0 &>/dev/null
ip link show br1 && ovs-vsctl del-br br1 &>/dev/null

# Deploy new OVS bridge
echo "--> Deploy new OVS bridge"
ovs-vsctl add-br br0 && ip link set br0 up
ovs-vsctl add-br br1 && ip link set br1 up
...
for GATEWAY in 172.31.216.1 10.197.11.191; do
	...
	if [ $? -eq 0 ]; then
		if [ ${GATEWAY} == "172.31.216.1" ]; then 
			...
			ovs-vsctl add-port br0 ${NIC}
			ip address add 172.31.218.47/22 dev br0
			# Type of port `eth1' is `Normal', which can't be configured ip address.
			ip route add default via ${GATEWAY} dev br0
			ovs-vsctl add-port br1 tun0 -- set Interface tun0 type=vxlan options:remote_ip=172.31.218.46
			...
			# Packages from 10.0.1.5 will be packaged attched 172.31.218.46 ip header.

			ip address add 10.0.1.5/24 dev br1
			ovs-vsctl show
			...
			ip address flush dev ${NIC}
			ovs-vsctl add-port br0 ${NIC}
			ip address add 10.197.11.201/24 dev br0
			ip route add default via ${GATEWAY} dev br0
			ovs-vsctl add-port br1 tun0 -- set Interface tun0 type=vxlan options:remote_ip=10.197.11.208
			ip address add 10.0.1.5/24 dev br1
			ovs-vsctl show
			echo "--> Deploy complete"
		fi
